package com.example.shopapp2.presentation.home.see_all.discounts
...
fun DiscountsSeeAllScreen(
    ...
    ModalBottomSheetLayout(
        ...
        Column(
            ...
            LazyColumn {
                itemsIndexed(homeViewModel.discountsProducts) { index, product ->
                    homeViewModel.onChangeDiscountsScrollPosition(index)

                    if ((index + 1) >= (homeViewModel.discountsPage.value * PAGE_SIZE)
                        && !homeViewModel.isLoading.value
                    ) {
                        homeViewModel.loadDiscountsNextPage()
                    }
