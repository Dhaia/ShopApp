package com.example.shopapp2.presentation.home.see_all.new
...
fun NewSeeAllScreen(
    ...
    ModalBottomSheetLayout(
        ...
        Column(
            ...
            LazyColumn {
                itemsIndexed(homeViewModel.newProducts) { index, product ->
                    homeViewModel.onChangeNewScrollPosition(index)

                    if ((index + 1) >= (homeViewModel.newPage.value * PAGE_SIZE)
                        && !homeViewModel.isLoading.value
                    ) {
                        homeViewModel.loadNewNextPage()
                    }
